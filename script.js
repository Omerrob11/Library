...
function handleRemoveBook(e) {
  ...
  let indexOfContainer = myLibrary.findIndex(
    (obj) => obj.counter === +contaierCounter
  );
...
function handleToggleRead(e) {
  ...
  haveYouReadItPar.textContent = book.finished
...
// form validation
const numPageInput = document.getElementById("num_pages");
const spanErrorMassage = document.querySelector(".error");
const form = document.querySelector(".form_container");

console.log(numPageInput);
numPageInput.addEventListener("input", (event) => {
  console.log(numPageInput.validity.typeMismatch);

  if (numPageInput.validity.valid) {
    spanErrorMassage.textContent = "";
    spanErrorMassage.className = "error";
  } else {
...
// it's for somereason not working , because it lookes like it still false

function showError() {
  if (numPageInput.validity.rangeUnderflow) {
    spanErrorMassage.textContent = "you need to enter number between 10-20";
  } else if (numPageInput.validity.rangeOverflow) {
    spanErrorMassage.textContent = "you need to enter number between 10-100";
  }

  spanErrorMassage.className = "error active";
}
